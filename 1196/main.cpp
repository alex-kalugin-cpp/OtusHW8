...
int main()
{
    // задача решается бинарным поиском по отсортированному вектору дат преподавателя + несложный предварительный фильтр
    // число записей преподавателя
     int N = 0;
     // минимальные и максимальная значения в списке преподавателя, нужны для предватительного фильтра
   ...
   int temp;
    for(int i=0;i<N;i++)
    {
       ...
       cin >> temp;
       // поскольку вектор отсортированный миниммум - первое значение максимум - последнее
      if(i == 0) min = temp;
      if(i == N-1) max = temp;
    ...
    auto tlowerB = endd;
    // для каждой даты студента из интервала мин макс используем бинарный поиск по датам преподавателя
     for(int i=0;i<M;i++)
     {
          cin >> temp;
