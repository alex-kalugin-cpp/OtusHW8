...
int main() {
  ...
  int N;
  string temp;
  // идея была про входным данным построить отношение игрок - игроки из одной
  // команды затем начиная с Isenbaev присваивать всем игрокам числа
  ...
  // берем список игроков одного уровня, смотрим с кем они играли и если у них
  // еще число не назначено - назначаем +1 к уровню текущего и формируем список
  // следующего уровня
  if (teams.find("Isenbaev") !=
      teams.end())  // грабли №2 забыл проверить наличие Исенбаева - запорол
                    // тест 3
  ...
  // вывод результатов
  for (auto it : result) cout << it.first << ' ' << it.second << endl;
  return 0;
}
